...
{% if "activations" in plugins and "config" in plugins["activations"] and "activations" in plugins["activations"]["config"] and len(plugins["activations"]["config"]["activations"]) %}{% for activation in plugins["activations"]["config"]["activations"] %}
	pageMod.PageMod({
		...
		{% if activation.has_key("all_frames") and activation["all_frames"] is True %}
			contentScriptFile: strArrToDataUrl(${json.dumps(["forge/app_config.js", "forge/all.js", "forge/jquery.js", "forge/jquery-ui.custom.js", "forge/jquery-noConflict.js"] + activation.scripts)}),
		{% end %} {% if not activation.has_key("all_frames") or activation["all_frames"] is False %}
			contentScriptFile: strArrToDataUrl(${json.dumps(["forge/app_config.js", "forge/all.js", "forge/jquery.js", "forge/jquery-ui.custom.js", "forge/jquery-noConflict.js", "forge/disable-frames.js"] + activation.scripts)}),
		{% end %}
